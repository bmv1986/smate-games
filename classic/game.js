...
let packageMediaPath = '';
    if (currentPackageFile) {
        // Пример: 'data/packages/package1.json' -> 'data/media/package1/'
        const packageName = currentPackageFile.match(/data\/packages\/(.+?)\.json/)?.[1] || 'unknown';
        packageMediaPath = `data/media/${packageName}/`;
    }

    // Загружаем общие аудио (тоже обновляем путь)
    timerAudio = new Audio('data/media/common/timer.mp3');
    timerAudio.loop = true;
    recordTimeAudio = new Audio('data/media/common/record_time.mp3');
